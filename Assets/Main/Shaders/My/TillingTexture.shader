Shader "CodeAmplify/AmpPannerTexture" {
	Properties {
		...
		_TileVec ("TillingSpeed", Vector) = (0,0,0,0)
		_OscilateRange ("OscilateRange", Range(0.0, 50.0)) = 5.0
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandardSpecular o) {
		    ...
		    half oscilateValue = sin(_OscilateRange * _Time);
		    float2 panners = panner(IN.uv_MainTex, oscilateValue, _TileVec);
			fixed4 c = tex2D (_MainTex, panners);
			o.Albedo = c.rgb; 
			float3 normalTex = UnpackNormal(tex2D(_BumpTexture, panners));
